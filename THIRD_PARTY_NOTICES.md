...
synthex as a whole is distributed under the **GNU General Public License v3.0**
(see `LICENSE`), because it contains code derived from Floos, which is itself
GPLv3 because it derives from XIUI.

## xitools

`synthex.lua`'s core synth-tracking logic (packet parsing for starting and
finishing a synth, skill-up tracking, the recipe search/browse UI) started as
a line-for-line port of the `crafty` tool inside **xitools** by lin (synthex
was itself named "crafty" until 3.0), along with `utils/packets.lua`,
`utils/ffxi.lua`, and the recipe data in `data/`
...
synthex's window theming (`libs/theme.lua`, copied) and font loading
...
- **Changes from Floos:** `libs/fonts.lua` was rewritten to load faces from
  ...
  `synthex/assets/fonts/`) instead of a bundled/downloaded copy; the rest of
  its structure is unchanged.

synthex is a separate addon and is not affiliated with or endorsed by the
Floos or XIUI projects.

## Fonts

No font files are bundled with synthex. The named fonts in `/synthex config` →
